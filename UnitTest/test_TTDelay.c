...
#define TTDELAY_RUN_TIMER_EXPECTATIONS      read_and_reset_counter_timer10_ExpectAndReturn(0); \
                                            read_and_reset_counter_timer10_ExpectAndReturn(0); \
                                            read_and_reset_counter_timer10_ExpectAndReturn(0)

#define TTDELAY_RUN_TASK_TIMER_EXPECTATIONS read_and_reset_counter_timer10_ExpectAndReturn(0); \
                                            read_and_reset_counter_timer10_ExpectAndReturn(0)
...
void test_add_delay_to_task(){
    read_and_reset_counter_timer10_IgnoreAndReturn(0);
    HAL_GetTick_ExpectAndReturn(0);
    HAL_GetTick_ExpectAndReturn(0);
    HAL_GetTick_ExpectAndReturn(0);
    ...
    HAL_GetTick_ExpectAndReturn(DELAY_TIME - 1);
    TTDelay_run();
    ...
    HAL_GetTick_ExpectAndReturn(2*DELAY_TIME - 1);
    TTDelay_run();
    TEST_ASSERT_EQUAL(1, delay_test_var);
...
void test_run_periodic_first_time_high_timer_count_should_run_once(){
    ...
    for (int i = 0 ; i < 10 ; i++){
        HAL_GetTick_ExpectAndReturn(10*DELAY_TIME - 1 + i);
        TTDelay_run();
        TEST_ASSERT_EQUAL(1, delay_test_var);
    }    
    // an additional run should not have anything scheduled
    HAL_GetTick_ExpectAndReturn(10*DELAY_TIME + 1);
    TTDelay_run();
    TEST_ASSERT_EQUAL(1, delay_test_var);
    // additional run scheduled here!
    HAL_GetTick_ExpectAndReturn(11*DELAY_TIME + 1);
    TTDelay_run();
    TEST_ASSERT_EQUAL(2, delay_test_var);
}
...
void test_full_run_cycle_with_rescheduled_high_priority(){
    ...
    HAL_GetTick_ExpectAndReturn(DELAY_TIME + 2);
    TTDelay_run();
    TEST_ASSERT_EQUAL(5, output_value);

    HAL_GetTick_ExpectAndReturn(DELAY_TIME + 3);
    TTDelay_run();
    TEST_ASSERT_EQUAL(10, output_value);
...
void test_cpu_usage_two_heavy_computing_tasks(){
    ...
    read_and_reset_counter_timer10_ExpectAndReturn(10);
    HAL_GetTick_ExpectAndReturn(0);                     // finding due tasks
    read_and_reset_counter_timer10_ExpectAndReturn(10); // start to run the task
    read_and_reset_counter_timer10_ExpectAndReturn(1000); // finish to run the task
    TTDelay_run();   // should run one "compute" (higher priority)
    TEST_ASSERT_EQUAL(1, a);

    read_and_reset_counter_timer10_ExpectAndReturn(10);
    HAL_GetTick_ExpectAndReturn(0);                     // finding due tasks
    read_and_reset_counter_timer10_ExpectAndReturn(20); // start to run the task
    read_and_reset_counter_timer10_ExpectAndReturn(3000); // finish to run the task
    TTDelay_run();   // should run the other "compute" task
    TEST_ASSERT_EQUAL(4, a);

    read_and_reset_counter_timer10_ExpectAndReturn(10);
    HAL_GetTick_ExpectAndReturn(0);                     // finding due tasks
    read_and_reset_counter_timer10_ExpectAndReturn(40); // start to run the task
    // this will not be part of the calculation as this is measured AFTER the calculation task ran
    read_and_reset_counter_timer10_ExpectAndReturn(40); // finish to run the task 
